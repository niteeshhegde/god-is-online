...
require 'connection.php';
 $city2= $_GET['city'];
 $category2= $_GET['pooja'];
echo'
<div>
<form class="form-inline" role="form" align="center" action="poojas2.php"   method="get">
...
<label for="sel1">LOCATION : </label>
                ...
                <option value="zirakpur">Zirakpur</option>
					...
					&#160;&#160;<label for="sel1">  NAME OF THE POOJA : </label>
      <select class="form-control" id="sel1" name="pooja">
...
if($city2==('all')){
	if($category2=='all'){
$query = "SELECT id, email, password, name, location, locality, lord ,description, image FROM temples ";
			$stmt=$conn->prepare($query);
			$stmt->execute();
$stmt->bind_result($ids, $emails, $passwords, $names, $locations, $localitys, $lords ,$descriptions, $images);
	}
	else{
		$query = "SELECT id, email, password, name, location, locality, lord ,description, image FROM temples WHERE poojas LIKE ? ";
			$stmt=$conn->prepare($query);
			$stmt->bind_param('s',$a);
			$a='%'.$category2.'%';
			$stmt->execute();
	...
	if($category2=='all'){
		$query = "SELECT id, email, password, name, location, locality, lord ,description, image FROM temples WHERE location = ? ";
			$stmt=$conn->prepare($query);
			$stmt->bind_param('s',$a);
			$a=$city2;
			$stmt->execute();
	...
	$query = "SELECT id, email, password, name, location, locality, lord ,description, image FROM temples WHERE (location = ? AND poojas LIKE ?)";
			$stmt=$conn->prepare($query);
