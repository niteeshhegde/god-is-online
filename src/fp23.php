...
if($_GET['code'])
{
	$get_id = $_GET['id'];
	$get_code = $_GET['code'];
	$query = "SELECT passreset FROM pandits WHERE id= ? ";
		$stmt=$conn->prepare($query);
		$stmt->bind_param('i',$a);
		$a=$get_id;
        $stmt->execute();
		$result= $stmt->bind_result($passreset);
    ...
    <div class="col-md-8">
      <section>      
        ...
        <hr>
            <form class="form-horizontal" action="fp24.php?code='.$get_code.'" method="POST" name="signip" id="signup" >        
