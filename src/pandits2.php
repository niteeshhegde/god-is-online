...
require 'connection.php';
 $city2= $_GET['city'];
 $category2= $_GET['ceremony'];
echo'
<div>
<form class="form-inline" role="form" align="center" action="pandits2.php"   method="get">
...
<label for="sel1">LOCATION : </label>
                ...
                <option value="zirakpur">Zirakpur</option>
					...
					&#160;&#160;<label for="sel1"> CEREMONY : </label>
      <select class="form-control" id="sel1" name="ceremony">
	  <option value="all">All</option>
	  	      <option value="MarriageCeremony">Marriage Ceremony</option>
  <option value="Upanayana">Upanayana</option>
  <option value="NamingCeremony">Naming Ceremony</option>
   <option value="Gruhapravesh">House Warming</option>
   <option value="SatyanarayanaKatha">Satyanarayana Katha</option>
</select>
...
if($city2==('all')){
	if($category2=='all'){
$query = "SELECT name,phone,location,MarriageCeremony,NamingCeremony,Upanayana,Gruhapravesh,SatyanarayanaKatha,image FROM pandits ";
			$stmt=$conn->prepare($query);
			$stmt->execute();
$stmt->bind_result($names,$phones,$locations,$MarriageCeremonys,$NamingCeremonys,$Upanayana,$Gruhapraveshs,$SatyanarayanaKathas,$images);
	}
	else{

$query = "SELECT name,phone,location,MarriageCeremony,NamingCeremony,Upanayana,Gruhapravesh,SatyanarayanaKatha,image FROM pandits WHERE ".$category2." LIKE ? ";
			$stmt=$conn->prepare($query);
			$stmt->bind_param('s',$a);
			$a="%".$category2."%";
			$stmt->execute();
...
$query = "SELECT name,phone,location,MarriageCeremony,NamingCeremony,Upanayana,Gruhapravesh,SatyanarayanaKatha,image FROM pandits WHERE location = ? ";
			$stmt=$conn->prepare($query);
			$stmt->bind_param('s',$a);
			$a=$city2;
			$stmt->execute();
...
$query = "SELECT name,phone,location,MarriageCeremony,NamingCeremony,Upanayana,Gruhapravesh,SatyanarayanaKatha,image FROM pandits WHERE (location = ? AND ".$category2." LIKE ? )";
			$stmt=$conn->prepare($query);
...
while($stmt->fetch()){
 ...
 <div class="row" align="center">
    <div class="col-sm-4" style="background-color:red;" >
      <img src="'.$images.'" alt="image" style="height:153px;">
    </div>
  ...
  <thead>
    <tr><td ><h4>'.$names.'</h4></td><td ><h4> Location : '.$locations.'</h4></td></tr>
  </thead>
  <tbody>
    <tr >
    </tr>
    <tr><td colspan="2">Phone : '.$phones.'</td></tr>
    <tr >
    </tr>
    <tr><td colspan="2">Skills : '.$MarriageCeremonys.'   '.$NamingCeremonys.'    '.$Upanayanas.'    '.$Gruhapraveshs.'    '.$SatyanarayanaKathas.' </td></tr>
