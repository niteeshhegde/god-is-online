...
$stmt=$conn->prepare($query);
	...
	$stmt->bind_result($ids, $emails, $passwords, $names, $locations, $localitys, $lords ,$descriptions, $images);
		...
		$query = "SELECT id, email, password, name, location, locality, lord ,description, image FROM temples WHERE lord = ? ";
			$stmt=$conn->prepare($query);
			$stmt->bind_param('s',$a);
			$a=$category2;
			$stmt->execute();
	...
	$query = "SELECT id, email, password, name, location, locality, lord ,description, image FROM temples WHERE (location = ? AND lord = ?)";
			$stmt=$conn->prepare($query);
